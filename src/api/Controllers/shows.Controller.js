import model from "#models/shows.Model"
...
const POST = async (req, res) => {
    try {
        const { show_name, show_category, release_date, duration, show_description } = req.body
        const { show_big_img, show_small_img, show_trailer } = req.files

        if(!show_big_img || !show_big_img || !show_trailer || !show_full){
...
const DELETE = async (req, res) => {
    try {
        ...
        const show = data.find(el => el.show_id == id)
        if(!show){
